...
if [ "x$RBENV_VERSION" != "x" ]; then
  ...
  curl -L https://raw.github.com/fesplugas/rbenv-installer/master/bin/rbenv-installer | /bin/sh
  export DEV_USERNAME_PATH_PREFIX="~/.rbenv/bin:~/.rbenv/shims"
  export PATH=$DEV_USERNAME_PATH_PREFIX:$PATH
...
if [ $? -ne 0 ]; then
  echo Until the following puppet bug http://projects.puppetlabs.com/issues/9862
  echo has been resolved, you may need to run the following as root:
  echo # groupadd puppet
fi
